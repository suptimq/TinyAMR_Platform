...
class Camera(SingletonConfigurable):
	...
	# config
	width = traitlets.Integer(default_value=224 * 2).tag(config=True)
	height = traitlets.Integer(default_value=224 * 2).tag(config=True)
	...
	def __init__(self, show=False, *args, **kwargs):
		...
		try:
			...
			self.value = image
			self.window_handle = cv2.namedWindow("CSI Camera", cv2.WINDOW_AUTOSIZE)
			self.start()
		except:
			self.stop()
			raise RuntimeError(
